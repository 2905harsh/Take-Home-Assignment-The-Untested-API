...
describe("taskService.getByStatus()", () => {
  ...
  it.failing(
    'does not match partial strings — "do" should not return todo or done tasks (Bug #1 — unfixed)',
    () => {
      // Bug #1: getByStatus uses .includes() so partial strings match incorrectly
      // 'do' matches both 'todo' and 'done' — this test documents the known broken behaviour
      // Fix: change t.status.includes(status) to t.status === status in taskService.js
      taskService.create({ title: "Todo task", status: "todo" });
      taskService.create({ title: "Done task", status: "done" });
...
describe("taskService.completeTask()", () => {
  ...
  it.failing(
    "does not change priority when completing a task (Bug #3 — unfixed)",
    () => {
      // Bug #3: completeTask hardcodes priority: 'medium'
      // so high/low priority tasks silently get downgraded on completion
      // Fix: remove priority field from the updated object in completeTask()
  ...
  it.failing(
    "low priority tasks also keep their priority after completion (Bug #3 — unfixed)",
    () => {
...
describe("GET /tasks", () => {
  ...
  it.failing("partial status filter returns nothing — not a substring match (Bug #1 — unfixed)", async () => {
    await createTask({ title: "Todo", status: "todo" });
    await createTask({ title: "Done", status: "done" });
...
describe("PATCH /tasks/:id/complete", () => {
  ...
  it.failing(
    "does not change the priority when completing a task (Bug #3 — unfixed)",
    async () => {
      // Bug #3: completeTask hardcodes priority: 'medium', this test documents the known failure
      const { body: task } = await createTask({
        title: "Urgent",
        priority: "high",
      });
